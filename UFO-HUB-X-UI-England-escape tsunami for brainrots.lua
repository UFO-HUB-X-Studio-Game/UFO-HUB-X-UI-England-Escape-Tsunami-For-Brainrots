...
--===== UFO HUB X • Auto Collect System [AAA1 MODEL - ABSOLUTE VER] =====
-- [RULE] : NEVER SHORTEN THE SCRIPT (FULL LENGTH)
-- [RULE] : SYSTEM MUST BE 100% ACCURATE TO PREVIOUS MODEL
-- [RULE] : START AT 5% DEFAULT
-- [RULE] : AAA1 SAVE SYSTEM INTEGRATED
-- [FIX]  : ITEM 2 UI DESIGN (SEARCH + GLOW BUTTONS)
...
_G.UFOX_AAA1["AutoCollect"] = _G.UFOX_AAA1["AutoCollect"] or {
    ...
    SpeedRel = SaveGet("SpeedRel", 0.05), -- เริ่มต้น 5% (0.05)
}
...
registerRight("Home", function(scroll)
    ...
    swBtn.MouseButton1Click:Connect(function() STATE.Enabled = not STATE.Enabled; SaveSet("Enabled", STATE.Enabled); updateSw(STATE.Enabled) end)
    updateSw(STATE.Enabled)

    -- [3] Row 2: Select Slots (BASED ON MODEL A V2)
    ...
    selectBtn.MouseButton1Click:Connect(function()
        ...
        optionsPanel.Name = "VA2_OptionsPanel"; optionsPanel.BackgroundColor3 = THEME.BLACK; optionsPanel.Position = UDim2.new(0.65, 0, 0.05, 0); optionsPanel.Size = UDim2.new(0.33, 0, 0.9, 0); optionsPanel.ZIndex = 100; corner(optionsPanel); stroke(optionsPanel, 2, THEME.GREEN)

        -- SEARCH BOX
        local search = Instance.new("TextBox", optionsPanel)
        search.Size = UDim2.new(1, -16, 0, 32); search.Position = UDim2.new(0, 8, 0, 8); search.BackgroundColor3 = Color3.fromRGB(20, 20, 20); search.PlaceholderText = "🔍 Search Slot..."; search.Text = ""; search.Font = "GothamBold"; search.TextSize = 12; search.TextColor3 = THEME.WHITE; corner(search, 6); stroke(search, 1.2, THEME.GREEN)

        local scroller = Instance.new("ScrollingFrame", optionsPanel)
        scroller.Size = UDim2.new(1, -10, 1, -55); scroller.Position = UDim2.new(0, 5, 0, 50); scroller.BackgroundTransparency = 1; scroller.ScrollBarThickness = 3; scroller.ScrollBarImageColor3 = THEME.GREEN; scroller.AutomaticCanvasSize = Enum.AutomaticSize.Y
        local llist = Instance.new("UIListLayout", scroller); llist.Padding = UDim.new(0, 6); llist.HorizontalAlignment = Enum.HorizontalAlignment.Center
        Instance.new("UIPadding", scroller).PaddingBottom = UDim.new(0, 10)

        local allButtons = {}
        local function makeItem(id, txt)
            local b = Instance.new("TextButton", scroller); b.Size = UDim2.new(0.92, 0, 0, 32); b.BackgroundColor3 = THEME.BLACK; b.Font = "GothamBold"; b.TextSize = 10.5; b.TextColor3 = THEME.WHITE; b.Text = txt; corner(b, 6); local st = stroke(b, 1.5, THEME.GREEN_DARK); st.Transparency = 0.5
            
            -- GLOW INDICATOR
            local glow = Instance.new("Frame", b); glow.BackgroundColor3 = THEME.GREEN; glow.Size = UDim2.new(0, 3, 1, 0); glow.Position = UDim2.new(0, 0, 0, 0); glow.Visible = false; corner(glow, 2)
            ...
            refresh(); table.insert(allButtons, {btn = b, ref = refresh})
        end
        ...
        search:GetPropertyChangedSignal("Text"):Connect(function()
            ...
            for _, btnData in ipairs(allButtons) do
                btnData.btn.Visible = (t == "" or btnData.btn.Text:lower():find(t))
            end
        end)
        ...
        inputConn = UserInputService.InputBegan:Connect(function(input)
            if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) then
                ...
                if not (m.X >= p.X and m.X <= p.X + s.X and m.Y >= p.Y and m.Y <= p.Y + s.Y) then closePanel() end
            end
        end)
    end)

    -- [4] Row 3: Metal Speed Slider (FIXED INPUT)
    ...
    local valLab = Instance.new("TextLabel", bar); valLab.BackgroundTransparency = 1; valLab.Size = UDim2.fromScale(1, 1); valLab.Font = "GothamBlack"; valLab.TextSize = 16; valLab.TextColor3 = THEME.WHITE; valLab.Text = math.floor(STATE.SpeedRel * 100 + 0.5) .. "%"
